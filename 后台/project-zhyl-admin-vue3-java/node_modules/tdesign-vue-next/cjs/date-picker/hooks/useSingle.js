...
function useSingle(props) {
  ...
  var inputValue = vue.ref(_common_js_datePicker_format.formatDate(value.value, {
  ...
  var inputProps = vue.computed(function () {
    return _objectSpread(_objectSpread({}, props.inputProps), {}, {
      ...
      onChange: function onChange(val) {
        ...
        if (!_common_js_datePicker_format.isValidDate(val, formatRef.value.format)) return;
        var newMonth = dayjs__default["default"](val).month();
        var newYear = dayjs__default["default"](val).year();
        var newTime = _common_js_datePicker_format.formatTime(val, formatRef.value.timeFormat);
      ...
      onEnter: function onEnter(val) {
        ...
        if (!_common_js_datePicker_format.isValidDate(val, formatRef.value.format) && !_common_js_datePicker_format.isValidDate(value.value, formatRef.value.format)) return;
        popupVisible.value = false;
        if (_common_js_datePicker_format.isValidDate(val, formatRef.value.format)) {
          onChange === null || onChange === void 0 ? void 0 : onChange(_common_js_datePicker_format.formatDate(val, {
            format: formatRef.value.format,
            targetFormat: formatRef.value.valueType
          }), {
            dayjsValue: _common_js_datePicker_format.parseToDayjs(val, formatRef.value.format),
            trigger: "enter"
          });
