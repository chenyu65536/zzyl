...
var _DatePicker = vue.defineComponent({
  ...
  setup: function setup(props2) {
    ...
    vue.watch(popupVisible, function (visible) {
      ...
      if (visible) {
        year.value = _common_js_datePicker_format.parseToDayjs(value.value, formatRef.value.format).year();
        month.value = _common_js_datePicker_format.parseToDayjs(value.value, formatRef.value.format).month();
        time.value = _common_js_datePicker_format.formatTime(value.value, formatRef.value.timeFormat);
      } else {
    ...
    function onCellMouseEnter(date) {
      ...
      inputValue.value = _common_js_datePicker_format.formatDate(date, {
        format: formatRef.value.format
      });
    }
    function onCellMouseLeave() {
      isHoverCell.value = false;
      inputValue.value = _common_js_datePicker_format.formatDate(cacheValue.value, {
        format: formatRef.value.format
      });
    }
    ...
    function onTimePickerChange(val) {
      ...
      inputValue.value = _common_js_datePicker_format.formatDate(nextDate, {
        format: formatRef.value.format
      });
    ...
    function onConfirmClick() {
      var nextValue = _common_js_datePicker_format.formatDate(inputValue.value, {
        format: formatRef.value.format
      });
      if (nextValue) {
        onChange === null || onChange === void 0 ? void 0 : onChange(_common_js_datePicker_format.formatDate(inputValue.value, {
          format: formatRef.value.format,
          targetFormat: formatRef.value.valueType
        }), {
          dayjsValue: _common_js_datePicker_format.parseToDayjs(inputValue.value, formatRef.value.format),
          trigger: "confirm"
        });
      } else {
        inputValue.value = _common_js_datePicker_format.formatDate(value.value, {
